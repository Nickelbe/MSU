...
def integral(f1, f2, ab, h):
    I = 0
    n = len(ab)
    for i in range(n-4):
        I += h * ( 14/45*(f1(ab[i]) - f2(ab[i])) + 64/45*(f1(ab[i+1]) - f2(ab[i+1])) + 24/45*(f1(ab[i+2]) - f2(ab[i+2])) + 64/45*(f1(ab[i+3]) - f2(ab[i+3]))
        + 14/45*(f1(ab[i+4]) - f2(ab[i+4])))
    return I
...
def adaptive(func1, func2, eps=eps):
    ...
    while alpha < beta:
        ...
        Ih2 = integral(func1, func2, s2, hi2)
        DELTAi = (Ih - Ih2) / (1 - 2 ** (-2))
